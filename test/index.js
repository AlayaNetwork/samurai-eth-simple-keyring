...
describe('simple-keyring', () => {
  ...
  describe('#signMessage', () => {
    ...
    it('reliably can decode messages it signs', async () => {
      ...
      const signatures = await Promise.all(addresses.map(async (accountAddress) => {
      ...
      signatures.forEach((sgn, index) => {
        ...
        assert.equal(adr, accountAddress, 'recovers address from signature correctly')
      })
    })
  })
  ...
  describe('getAppKeyAddress', function () {
    it('should return a public address custom to the provided app key origin', async function () {
      ...
      const appKeyAddress = await simpleKeyring.getAppKeyAddress(address, 'someapp.origin.io')
    ...
    it('should return different addresses when provided different app key origins', async function () {
      ...
      const appKeyAddress1 = await simpleKeyring.getAppKeyAddress(address, 'someapp.origin.io')

      assert(ethUtil.isValidAddress(appKeyAddress1))

      const appKeyAddress2 = await simpleKeyring.getAppKeyAddress(address, 'anotherapp.origin.io')
    ...
    it('should return the same address when called multiple times with the same params', async function () {
      ...
      const appKeyAddress2 = await simpleKeyring.getAppKeyAddress(address, 'someapp.origin.io')
  ...
  describe('signing methods withAppKeyOrigin option', function () {
    it('should signPersonalMessage with the expected key when passed a withAppKeyOrigin', async function () {
      ...
      const simpleKeyring = new SimpleKeyring([testAccount.key])
      const sig = await simpleKeyring.signPersonalMessage(address, message, {
